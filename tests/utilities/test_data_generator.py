...
class LesserTestDataGenerator:
    ...
    def __init__(self, instance_url: str = "https://lesser.example.com", 
    ...
    def generate_actor(self, actor_type: str = "Person") -> Dict[str, Any]:
        ...
        actor = {
            ...
            "published": fake.date_time_between(start_date='-2y', end_date='now').isoformat() + 'Z',
            "url": f"{self.instance_url}/@{username}",
            "endpoints": {
                "sharedInbox": f"{self.instance_url}/inbox"
            }
        }
        ...
        if random.random() > 0.7:
            actor["attachment"] = [
                ...
                },
                {
                    "type": "PropertyValue",
                    "name": "Pronouns",
                    "value": random.choice(["he/him", "she/her", "they/them", "ze/zir"])
                }
            ]
    ...
    def generate_note(self, author: Optional[Dict[str, Any]] = None,
                     in_reply_to: Optional[str] = None,
                     content_length: str = "medium") -> Dict[str, Any]:
        ...
        note = {
            ...
            "cc": [author["followers"]],
            "sensitive": random.choice([True, False]) if random.random() > 0.9 else False,
        ...
        if random.random() > 0.6:
            ...
            for i in range(random.randint(1, 4)):
                attachment = {
                    "type": "Document",
                    "mediaType": random.choice(["image/jpeg", "image/png", "image/gif", "video/mp4"]),
    ...
    def generate_activity(self, activity_type: str = "Create",
                         actor: Optional[Dict[str, Any]] = None,
                         object: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if activity_type == "Create":
            ...
            if not object and self.actors:
                object = random.choice([a for a in self.actors if a["id"] != actor["id"]])
            elif not object:
    ...
    def generate_conversation_thread(self, depth: int = 5, 
                                   participants: int = 3) -> List[Dict[str, Any]]:
        ...
        for i in range(depth - 1):
            ...
            reply = self.generate_note(
                author=replier,
                in_reply_to=current_parent["id"]
            )
            thread.append(reply)
    ...
    def generate_follow_network(self, actors_count: int = 10,
                               follow_probability: float = 0.3) -> Dict[str, Any]:
        ...
        for actor in network_actors:
            for potential_followee in network_actors:
                if actor["id"] != potential_followee["id"] and random.random() < follow_probability:
                    follow = self.generate_activity(
                        activity_type="Follow",
                        actor=actor,
                        object=potential_followee
                    )
    ...
    def generate_timeline_data(self, days: int = 7, 
                              posts_per_day: int = 20) -> List[Dict[str, Any]]:
            ...
            for hour in range(24):
                ...
                for _ in range(post_count):
                    ...
                    if content_type == "note":
                        ...
                        note = self.generate_note(author=actor, in_reply_to=parent["id"])
                        activity = self.generate_activity("Create", actor, note)
                    ...
                    activity["published"] = hour_start.replace(
                        minute=minutes, 
                        second=seconds
                    ).isoformat() + 'Z'
...
-----END PUBLIC KEY-----"""
    ...
    def create_test_account(self, username: str, email: str, password: str) -> Dict[str, Any]:
        """Create a test account via the API"""
        if not self.instance_url.startswith(('http://', 'https://')):
            print(f"Skipping API call - instance URL not a valid HTTP endpoint: {self.instance_url}")
            return {
                "username": username,
                "email": email,
                "status": "skipped",
                "reason": "Not a valid HTTP endpoint"
            }
    ...
    def populate_instance(self, accounts: int = 5, posts_per_account: int = 10,
                         follow_probability: float = 0.3) -> Dict[str, Any]:
...
def main():
    ...
    print(f"  - Created network with {network['stats']['total_follows']} follow relationships")
