...
if (args.length < 3) {
    ...
    console.error('Usage: node generate_bootstrap_data.js <username> <domain> <table_name> [admin]');
    console.error('\nExample:');
    console.error('  node generate_bootstrap_data.js alice example.com prod-table');
    console.error('  node generate_bootstrap_data.js admin example.com prod-table admin');
    console.error('\nParameters:');
    console.error('  username    - Required. The username to create (no defaults for security)');
    console.error('  domain      - Required. Your Lesser domain (e.g., lesser.app)');
    console.error('  table_name  - Required. DynamoDB table name');
    console.error('  admin       - Optional. Pass "admin" to create an admin user');
    process.exit(1);
}
...
if (username.match(/[^a-zA-Z0-9_-]/)) {
    console.error('ERROR: Username can only contain letters, numbers, underscore and hyphen');
    process.exit(1);
}
...
const securePassword = generateRandom(16);

// Hash the password using bcrypt with cost factor 12 (matching Go implementation)
const passwordHash = bcrypt.hashSync(securePassword, 12);
...
// Write JSON files
fs.writeFileSync(path.join(outputDir, 'actor.json'), JSON.stringify(actorData, null, 2));
fs.writeFileSync(path.join(outputDir, 'user.json'), JSON.stringify(userData, null, 2));
fs.writeFileSync(path.join(outputDir, 'oauth_client.json'), JSON.stringify(oauthClientData, null, 2));
...
Initial Password: ${securePassword}
...
${ jwtSecretSource === 'generated' ? `⚠️  CRITICAL: You MUST set this JWT secret in your Lambda environment:
   AWS Lambda: Set environment variable JWT_SECRET=${jwtSecret}
   Local: export JWT_SECRET=${jwtSecret}

   Without this, authentication WILL NOT WORK!` : '✓ Using JWT_SECRET from environment'}
...
console.log(`4. Generate JWT token: JWT_SECRET=${jwtSecret} node create_token.js`);
