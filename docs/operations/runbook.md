...
- Trusted forwarding roots: set `CRAWLER_TRUSTED_PROXY_CIDRS` to the proxy CIDRs allowed to supply
  `X-Forwarded-For`; without a trusted proxy hop, crawler protection treats the client IP as `unknown`.
