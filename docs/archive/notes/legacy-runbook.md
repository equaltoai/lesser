# Lesser Production Runbook (Archived Template)

> Archived: historical template content that may not match current naming conventions or infra behavior. Do not treat this as current operational guidance.
...
# Advanced: run CDK directly (requires context)
cd infra/cdk
AWS_PROFILE=<profile> cdk diff --all \
  --context app=<app> \
  --context baseDomain=<base-domain> \
  --context hostedZoneId=<hosted-zone-id> \
  --context stage=live

AWS_PROFILE=<profile> cdk deploy <app>-shared <app>-live \
...
aws lambda update-function-code --function-name lesser-production-api --zip-file fileb://bin/api.zip
```
