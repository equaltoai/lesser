...
| `pkg/storage/repositories/analytics_repository.go:633` (1) | hashtag metadata scan by `SK = METADATA` (filters `LastUsed`) | **STOP-AND-REPORTED in wave part 2 (batch S3, #1469, issue #1501, 2026-08-28) — remains baselined.** Caller sweep: `TrendingRepository.GetRecentHashtags` has one live production caller — the scheduled `cmd/trend-aggregator` Lambda (CDK inventory `trend-aggregator`, daily `cron(0 2 * * ? *)`, `aggregateHashtagTrends` → `GetRecentHashtags(ctx, since, 1000)`) — and one dead chain (`scoreHashtagSuggestions` ← `GenerateSearchSuggestions`, zero production callers). The rows it reads (`SK = METADATA`) have no live writer (`IndexHashtag` has zero production callers), so the read returns empty in production and a keyed read on the never-populated index would silently return nothing — per issue #1501 directive (c) the honest resolution (delete the trend-aggregator's dead hashtag step + the read, or bound in place) is reported and the orchestrator decides; the baseline entry stays |
...
- **Eliminated in wave part 2 (batch S3, #1469, issue #1501, 2026-08-28 — the analytics/search leftover class, part 1):** `pkg/storage/repositories/hashtag_repository.go` (baselined `goDynamoDBQueryScan: 1`) is gone — `queryHashtagMetadataByDateRange` (the literal `.Scan` at `:1297`, `Filter("SK","=",METADATA)` + `LastUsed` date filters) is DELETED together with its whole dead read family. Caller sweep: `GetSuggestedHashtags`, `HashtagRepository.GetRecentHashtags`, and `GetHashtagsByTimeRange` (the only callers) all have zero production callers — the live suggestions/trending paths go through `SearchRepository.GetSearchSuggestions`/`TrendingRepository` instead; the metadata rows the family reads are only written by `IndexHashtag` (zero production callers), so the reads were permanently empty and no keyed read can answer them. Deleted per the batch-L zero-caller precedent: the helper, the three methods, `GetSuggestedHashtags` (interfaces.HashtagRepository + mocks + inmemory), the orphaned `convertHashtagsToTrendingHashtags`, and their tests (the permissive sweep tests dropped their now-dead `Scan` stubs). This closes the batch-E-reverted no-live-writer story for the hashtag_repository.go member of the metadata family (the `TrendingRepository.GetRecentHashtags` member is STOP-AND-REPORTED below; `getCandidateHashtags` was deleted with the whole dead enhanced TrendingEngine — see the `goDynamoDBAllNoKey` table). **STOP-AND-REPORTED — remains baselined (`analytics_repository.go: 2`):** `GetUserSearchHistory` (`:1122`, raw `Limit(limit)`, zero-limit exposure) and `GetPopularSearchQueries` (batch-E-reverted GSI8 delegation) — caller sweep shows the sole caller chain `GenerateSearchSuggestions` → `scoreUserHistory`/`scorePopularQueries` has **zero production callers** (the live suggestions route `/api/v1/accounts/search/suggestions` goes through `notes.Service.GetSearchSuggestions` → `SearchRepository.GetSearchSuggestions`), so both reads are dead code. The issue's assumed designs (keyed `.All` + `Limit(n>0)` clamp for `GetUserSearchHistory`; 7-day `SEARCH_LOG#<date>` bounded walk + in-memory aggregation for `GetPopularSearchQueries` with `TestGetPopularSearchQueries_WindowSemantics` surviving unchanged) presuppose a live read; the pinned test seeds `SearchQuery` rows (`PK = USER#<id>`) while the SEARCH_LOG partitions hold `SearchAnalytics` rows, so the test cannot pass literally unchanged under that design. Per the brief's stop-and-report rule the two honest dispositions (delete the dead chain, or convert for a future wiring and re-shape the test seeding to `SearchAnalytics` while preserving every window assertion) are reported for the orchestrator to decide; entries stay baselined meanwhile. SEARCH_LOG rows verified to carry every field the aggregation needs (Query, ResultCount, Timestamp, UserID) and the per-day partitions ARE populated by the live `RecordSearchWithPrivacy` writer (`cmd/api/handlers/search.go:266,511`) — the SEARCH_LOG-walk mechanism itself is viable if the orchestrator chooses conversion.
