...
- **Bounded in wave part 2 (batch N3, #1469, 2026-08-27 — final keyed-read batch):** the remaining BOUNDED-QUERY class in the named search/announcement/cost-metrics files (workstream 1), the keyed `.Count()` class tracked above plus the Count gaps the N1/N2 dispositions missed (workstream 2), and a remainder sweep of every other file (workstream 3). Same mechanisms as N1/N2 (`walkKeyedPages` = `Limit(500)`/page via `AllPaginated`, 100-page cap, fail-closed `errBoundedPageCapExceeded`; clamps always issue `Limit(n>0)`).
  ...
  - **Workstream 3 — remainder sweep (wave-completeness proof):** every file not dispositioned by N1/N2 was enumerated multiline/AST-aware (669 read terminals package-wide: 395 `.All`, 31 `.Count`, 226 `.First`, 17 `.Scan`; count is tool-definition-approximate — reproduced here with a comment/string-aware tokenizer counting `All(`/`Count(`/`First(`/`Scan(` method calls matched across line breaks, non-test `.go` files in `pkg/storage/repositories` only, and prior toolings reported 705/681 — the per-file dispositions below are unaffected; `.First` terminals are bounded by `firstInternal` limit 1 — recorded, not converted). Per-file result: (a) fixed here beyond the named files — the two degenerate `if limit > 0` gates: `federation_repository.go:2658` `GetDetailedMetricsByPeriod` and `media_metadata_repository.go:102` `GetMediaMetadataByStatus` (both floored to 500); (b) verified-bounded/clean — `domain_pagination_helpers.go` (Limit(safeLimit+1), clamps 20/100), `severance_repository.go` (all 4 sites Limit(limit+1), clamps 1/100), `relationship_pagination_helpers.go` (`NormalizePaginationLimit`), `notification_helpers.go` `buildPaginatedNotificationQuery` (clamp 20/100 + `Limit(opts.Limit+1)`), `base_repository.go:929` `Exists` (point read), `hashtag_trending_engine.go:872` (`strings.Count` — not a DB read), `user_repository.go:1189` `GetVouch` (PK+SK point read, matches the N2 out-of-scope rule), `instance_counts.go` (5 — deliberate offline recount tool, baselined, untouched); (c) **the sweep uncovered a class too large for this batch and it is itemized below as the N4 batch — NOT half-fixed**.
  - **Named N4 batch — the keyed whole-partition remainder (108 sites, 52 files):** the sweep found the BOUNDED-QUERY class continues beyond the named files — keyed `.All()`/`.Count()` reads with no Limit, mostly request-path (auth, search, timeline, relationship, fan-out) plus cleanup (read-then-delete) and aggregation clusters. These are NOT fixed in N3; they are recorded here with the file:site list so the N4 batch owns them. `account_repository.go:2823` (GetAccountPreferences), `account_repository_auth.go:268,520,562,913,1307` (GetUserSessions/ClearLoginAttempts/GetLoginAttemptCount/GetUserDevices/GetLinkedProviders), `account_repository_refresh_tokens.go:253,272`, `account_repository_search.go:103,462`, `account_repository_social.go:452,681`, `account_repository_webauthn.go:101,626,659`, `agent_access_lease_repository.go:282`, `agent_share_repository.go:150,165`, `alert_repository.go:358,392` (keyed Count), `auth_refresh_token_repository.go:245,305,339`, `auth_repository.go:233`, `base_repository.go:913,1363` (keyed Count + FindByPK), `block_repository.go:236,251` (keyed Count), `bookmark_repository.go:270`, `community_note_repository.go:369`, `conversation_repository.go:726`, `conversation_state_repository.go:447`, `csrf_repository.go:221,256`, `draft_repository.go:491,513,539`, `enhanced_base_repository.go:348` (keyed Count), `enhanced_pattern_repository.go:713,726`, `export.go:193`, `feature_repository.go:232` (keyed Count), `featured_tag_repository.go:255`, `federation_activity_repository.go:63`, `federation_repository.go:173,456,1509,2611`, `filter_repository.go:209,396,438`, `graphql_stream_subscription_repository.go:102,140,176`, `hashtag_repository.go:313,410,433,486` (`:410` keyed Count), `import.go:216`, `import_export_simple_helpers.go:112`, `instance_health_repository.go:455`, `instance_repository.go:793,987`, `like_repository.go:162,339,356` (keyed Count), `list_repository.go:121,326,543,588,595` (`:326,588` keyed Count), `mute_repository.go:232,247` (keyed Count), `oauth_helpers.go:699,716,732`, `oauth_session_repository.go:80`, `pattern_repository.go:167`, `promo_package_repository.go:437,460,486`, `publication_member_repository.go:64`, `quote_repository.go:299,330` (`:299` keyed Count), `rate_limit_repository.go:228,326`, `recovery_repository.go:67,231,306`, `relationship_repository.go:528,549,595,610,1334` (keyed Count), `social_repository.go:647,1156` (keyed Count), `streaming_connection_repository.go:236,731,755` (`:731,755` keyed Count), `threat_intel_repository.go:234`, `thread_repository.go:118,232`, `timeline_repository.go:289` (keyed Count), `wallet_repository.go:284`, `websocket_subscription_manager_repository.go:141,168,210,252` (`:252` `GetUserConnections` — keyed gsi2 `USER#<id>`/`CONNECTION#` read). **Wave-completeness statement:** N3 closes the named keyed-read classes (workstream 1 files) and the keyed-`.Count()` class including the N1/N2 doc-gap sites; the sweep's remaining keyed whole-partition class is itemized above as N4. The wave's keyed-read closure is complete when N4 lands; the baselined key-less scan backlog (analytics `GetUserSearchHistory`/`GetPopularSearchQueries`/`GetRecentHashtags`, moderation `GetFlag`, object `QueryScan`/`CountWithdrawnQuotes`, query_utils `QueryWithPKAndSKPrefix` `useFilter` branch) remains the milestone-roadmap's, untouched.
