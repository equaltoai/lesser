...
| `pkg/storage/repositories/moderation_repository.go:2095` (1) | `GetFlag`: `SK LIKE %#id` scan | **Kept baselined (bounded, NOT latent):** reached from the event path via `DeleteFlag` ← `cmd/activity-processor` (flag-undo handling), so it is not a LATENT-class site; the read is already bounded (`Limit 10`) and no existing index serves a flag-ID lookup without the object key, so it stays for the request/event-path batch. Disclosed in wave part 2 (batch L, #1469, 2026-08-27) after a deletion attempt surfaced the live caller chain |
