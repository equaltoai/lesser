...
| `pkg/storage/repositories/analytics_repository.go:633` (1) | hashtag metadata scan by `SK = METADATA` (filters `LastUsed`) | **Fixed in wave part 2 (batch E, #1469):** `Hashtag` gained GSI1 global listing (`HASHTAGS#ALL` / `<LastUsed RFC3339>#<name>`) maintained by `UpdateKeys` (the only writer, `IndexStatusHashtags`, goes through `ValidateAndCreate` → `Create` → `UpdateKeys`); `GetRecentHashtags` queries `Index("gsi1")` with a `gsi1SK >=` window. Legacy rows without gsi1 are not returned until next written (hashtag metadata is continuously written) |
...
| `pkg/storage/repositories/hashtag_trending_engine.go:351` (1) | `getCandidateHashtags`: `SK = METADATA` + `LastUsed` filter | **Fixed in wave part 2 (batch E, #1469):** queries the new `Hashtag` GSI1 global listing (`HASHTAGS#ALL` / `<LastUsed RFC3339>#<name>`) with a `gsi1SK >=` window; `UsageCount` stays a post-limit filter, so candidates are now the most-recent-in-window hashtags meeting the threshold (previously a limit-bounded unordered table sample with filters) |
...
| `pkg/storage/repositories/moderation_repository.go:2945,2957,2972` (3) | reports/flags by `AssignedTo` (no assignee index) | **Fixed in wave part 2 (batch E, #1469):** `Report` gained GSI4 assignee index (`ASSIGNED#<assignee>` / `<status>#REPORT#<createdAtUnix>`) maintained by `UpdateKeys` (every writer — `CreateReport`, `AssignReport`, `UnassignReport`, `UpdateReportStatus` — already calls it; unassigned reports carry empty GSI4 keys); `GetPendingModerationCount` counts open/in-progress assigned reports via keyed `gsi4SK begins_with` ranges. The flags branch queries the existing GSI2 (`FLAG_STATUS#pending`) with the assignee filter preserved — the `Flag` model has no `AssignedTo` attribute, so that branch always counted zero (preserved). Legacy assigned reports without gsi4 are not counted until next written (assign/status updates refresh keys) |
