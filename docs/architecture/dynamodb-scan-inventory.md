...
- **Bounded in wave part 2 (batch N1, #1469, 2026-08-27):** the BOUNDED-QUERY class in the three owned files — keyed whole-partition `.All()` reads (a partition key present, but no enforced Limit). These are not scans and never entered the scan gates; the wave's exit criterion disposes every one. Dispositions (mechanism per site; no GSI/schema/model changes):
  ...
  - `query_utils.go` (8 sites) — the six conditional-limit builders (`UserRelationshipQuery`, `TimeRangeQuery`, `GSIStatusQuery`, `QueryByGSI`, `QueryWithPrefix`, `GenericList`) plus `QueryBuilder.Execute` previously skipped `Limit` when the caller passed 0 (unbounded); all now clamp via `sanitizeQueryLimit` (default 50 / hard max 100) and always apply `Limit(opts.Limit+1)`. `QueryWithPKAndSKPrefix`'s keyed `Where` branch (used by `AccountRepository.GetBlocks`) is now a page-capped walk (500/page, 100-page cap); its `useFilter` `.Scan` branch remains baselined under `goDynamoDBQueryScan` (`query_utils.go: 1`) untouched.
