...
- **Bounded in wave part 2 (batch N3, #1469, 2026-08-27 — final keyed-read batch):** the remaining BOUNDED-QUERY class in the named search/announcement/cost-metrics files (workstream 1), the keyed `.Count()` class tracked above plus the Count gaps the N1/N2 dispositions missed (workstream 2), and a remainder sweep of every other file (workstream 3). Same mechanisms as N1/N2 (`walkKeyedPages` = `Limit(500)`/page via `AllPaginated`, 100-page cap, fail-closed `errBoundedPageCapExceeded`; clamps always issue `Limit(n>0)`).
  ...
  - `websocket_cost_repository.go` (3 raw) — floors (`limit <= 0` → 500, no max — the internal aggregation callers pass 10000 at `websocket_cost_repository.go:338,408` and expect the full window): `queryByGSIWithTimeRange` (backing `ListByConnection`/`ListByUser`), `queryBudgetsByGSI`, `queryAggregationsByGSI` — the `if limit > 0 { Limit(limit) }` gate previously skipped Limit entirely at `limit <= 0`.
  - `federation_cost_repository.go` (3 raw) — floors (`limit <= 0` → 500): `GetFederationCosts` (per-month gsi1 bucket; the `if limit > 0` gate previously skipped Limit), `GetFederationCostsByActivityType` (`Limit(0)`-compiles-to-no-limit), `GetActiveBudgets` (no max — the internal callers pass 1000 at `GetBudgetsOverLimit`/`GetBudgetsNeedingAlerts` and expect the full active set).
  - `route_optimizer_repository.go` (5 raw) — floors (`limit <= 0` → 500, no max — the internal callers pass 1000 at `route_optimizer_repository.go:190,252`): `GetRouteResults`, `GetRecentResults`, `GetOptimizationDecisions`, `GetMetricsInRange` (both route and GSI branches).
