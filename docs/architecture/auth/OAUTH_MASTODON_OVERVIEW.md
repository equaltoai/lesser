...
- **Lesser implementation**: secrets are hashed at rest and are only returned at `POST /api/v1/apps` time; if a secret is lost or needs rotation, register a new app (new `client_id`/`client_secret`) and update clients.
