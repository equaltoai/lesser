...
func (r *mutationResolver) IncorporateSoul(ctx context.Context, agentID string, targetAgentUsername string) (*model.SoulInventoryItem, error) {
	...
	soul, err := service.Incorporate(ctx, claims.Username, targetAgentUsername, agentID)
	if err != nil {
		if r.Logger != nil {
			r.Logger.Error(
				"failed to incorporate soul",
				zap.String("username", claims.Username),
				zap.String("target_agent_username", targetAgentUsername),
	...
	return toGraphQLSoulInventoryItem(*soul), nil
}
