...
func (s *actorServiceImpl) GetAccountWithStats(ctx context.Context, username string) (*models.Account, error) {
	...
	followerCount := 0
	followers, _, err := s.store.Relationship().GetFollowers(ctx, username, 1, "")
	...
	// Get following count
	followingCount := 0
	following, _, err := s.store.Relationship().GetFollowing(ctx, username, 1, "")
	...
	// Get statuses count
	statusesCount := 0
	objects, _, err := s.store.Object().GetObjectsByActor(ctx, actor.ID, "", 1)
