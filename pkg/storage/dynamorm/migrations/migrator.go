...
func (m *Migrator) UpdateMigrationStatus(ctx context.Context, status *MigrationStatus) error {
	status.PK = "MIGRATION#STATUS"
	status.SK = "CURRENT"
...
func (m *Migrator) recordMigrationHistory(ctx context.Context, migration Migration, status string, duration time.Duration, execErr error) error {
...
func (m *Migrator) calculateChecksum(migration Migration) string {
	...
	h.Write([]byte(fmt.Sprintf("%d", migration.Version())))
...
func (m *Migrator) acquireLock(ctx context.Context) error {
	...
	if err != nil {
		...
		existingLock.PK = "MIGRATION#LOCK"
...
func (m *Migrator) GetMigrationHistory(ctx context.Context) ([]*MigrationHistory, error) {
	var histories []MigrationHistory
	err := m.db.Model(&MigrationHistory{}).
		Where("PK", "=", "MIGRATION#HISTORY").
		OrderBy("Version", "DESC").
