...
func TestMediaRepositoryUpdateMediaPublishedStateIsFieldScoped(t *testing.T) {
	...
	err := repo.UpdateMediaPublishedState(ctx, "m1", "published/media/m1.png", "https://cdn.example.test/published/media/m1.png", now)
...
func TestMediaRepositoryUpdateMediaEditorialStateIsFieldScoped(t *testing.T) {
	...
	repo := NewMediaRepository(db, "test-table", zap.NewNop(), nil)
	err := repo.UpdateMediaEditorialState(ctx, "m1", models.EditorialLifecycleWithdrawn, "")
	require.NoError(t, err)
	db.AssertExpectations(t)
	query.AssertExpectations(t)
	update.AssertExpectations(t)
...
func TestMediaRepositoryUpdateMediaEditorialStateSupersededNamesSuccessor(t *testing.T) {
	...
	repo := NewMediaRepository(db, "test-table", zap.NewNop(), nil)
	require.NoError(t, repo.UpdateMediaEditorialState(ctx, "m1", models.EditorialLifecycleSuperseded, "m2"))
	require.Error(t, repo.UpdateMediaEditorialState(ctx, "m1", models.EditorialLifecycleSuperseded, "  "))
	db.AssertExpectations(t)
	query.AssertExpectations(t)
	update.AssertExpectations(t)
}
