...
func unmarshalWithDynamORMCompat(item map[string]any, out any) error {
	...
	// Enhanced field mapping with TableTheory tag support
	for i := 0; i < outType.NumField(); i++ {
		...
		// Get field name from TableTheory tag or JSON tag or field name
...
// getFieldName extracts field name from struct tags (theorydb, json) or uses field name
func getFieldName(field reflect.StructField) string {
	// Prefer TableTheory tags when present.
	if tag, ok := field.Tag.Lookup("theorydb"); ok {
		parts := strings.Split(tag, ",")

		// Tags encode the attribute name explicitly, e.g. "pk,attr:PK" or "attr:objectID".
		...
		// Older tags may be a single attribute name (e.g. "id", "version", "description").
		if len(parts) == 1 {
			switch tag {
