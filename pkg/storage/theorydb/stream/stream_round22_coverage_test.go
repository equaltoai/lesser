...
type round22Model struct {
	...
	When     time.Time        `theorydb:"attr:when"`
	Optional *string          `theorydb:"attr:optional"`
	Nested   *round22Nested   `theorydb:"attr:nested"`
	Tags     []string         `theorydb:"attr:tags"`
	Meta     map[string]any   `theorydb:"attr:meta"`
	AnyField any              `theorydb:"attr:anyField"`
	Int8     int8             `theorydb:"attr:int8"`
	BadBool  bool             `theorydb:"attr:badBool"`
	Ignored  string           `json:"-"`
	Struct   round22PlainNest `theorydb:"attr:struct"`
}
