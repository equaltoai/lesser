...
func (rc *RelayCost) UpdateKeys() {
	...
	rc.SK = fmt.Sprintf("TS#%s#%s", rc.Timestamp.Format("20060102150405"), rc.RequestID)

	// GSI1: Query costs by relay URL
	rc.GSI1PK = fmt.Sprintf("RELAY_COSTS#%s", rc.RelayURL)
	rc.GSI1SK = fmt.Sprintf("%s#%s", rc.Timestamp.Format("20060102150405"), rc.OperationType)

	// GSI2: Query costs by time period (for aggregation)
	date := rc.Timestamp.Format("20060102")
...
func (rm *RelayMetrics) UpdateKeys() {
	...
	rm.SK = fmt.Sprintf("WINDOW#%s", rm.WindowStart.Format("20060102150405"))

	// GSI1: Query metrics by relay URL across all periods
	rm.GSI1PK = fmt.Sprintf("RELAY_METRICS#%s", rm.RelayURL)
	rm.GSI1SK = fmt.Sprintf("%s#%s", rm.Period, rm.WindowStart.Format("20060102150405"))
}
