...
type NotificationCostTracking struct {
	...
	DeliveryMethod        string    `json:"delivery_method"`        // email, push, websocket, sms
	...
	EmailCostMicroCents     int64 `json:"email_cost_micro_cents"`
	PushCostMicroCents      int64 `json:"push_cost_micro_cents"`
	SMSCostMicroCents       int64 `json:"sms_cost_micro_cents"`
	...
	EmailCostDollars     float64 `json:"email_cost_dollars"`
	PushCostDollars      float64 `json:"push_cost_dollars"`
	SMSCostDollars       float64 `json:"sms_cost_dollars"`
...
func (n *NotificationCostTracking) UpdateKeys() {
	timestampStr := n.Timestamp.Format("20060102150405")
	dateStr := n.Timestamp.Format("20060102")
	...
	// GSI1 - User queries
	n.GSI1PK = fmt.Sprintf("USER#%s", n.Username)
...
func (n *NotificationCostTracking) CalculateDollarAmounts() {
	...
	n.EmailCostDollars = float64(n.EmailCostMicroCents) / microCentsToDollars
	n.PushCostDollars = float64(n.PushCostMicroCents) / microCentsToDollars
	n.SMSCostDollars = float64(n.SMSCostMicroCents) / microCentsToDollars
...
func (n *NotificationCostTracking) AddCost(method string, costMicroCents int64) {
	switch method {
	case "email":
		n.EmailCostMicroCents += costMicroCents
	case "push":
		n.PushCostMicroCents += costMicroCents
	case "sms":
		n.SMSCostMicroCents += costMicroCents
	case "websocket":
	...
	n.TotalCostMicroCents = n.EmailCostMicroCents + n.PushCostMicroCents + 
		n.SMSCostMicroCents + n.WebSocketCostMicroCents + 
...
type NotificationCostAggregation struct {
	...
	DeliveryMethod  string    `json:"delivery_method"`   // email, push, websocket, sms, all
	...
	TotalEmailCostMicroCents     int64 `json:"total_email_cost_micro_cents"`
	TotalPushCostMicroCents      int64 `json:"total_push_cost_micro_cents"`
	TotalSMSCostMicroCents       int64 `json:"total_sms_cost_micro_cents"`
	...
	TotalEmailCostDollars     float64 `json:"total_email_cost_dollars"`
	TotalPushCostDollars      float64 `json:"total_push_cost_dollars"`
	TotalSMSCostDollars       float64 `json:"total_sms_cost_dollars"`
...
func (n *NotificationCostAggregation) CalculateDollarAmounts() {
	...
	n.TotalEmailCostDollars = float64(n.TotalEmailCostMicroCents) / microCentsToDollars
	n.TotalPushCostDollars = float64(n.TotalPushCostMicroCents) / microCentsToDollars
	n.TotalSMSCostDollars = float64(n.TotalSMSCostMicroCents) / microCentsToDollars
...
func (b *NotificationCostTrackingBuilder) WithCosts(emailCost, pushCost, smsCost, websocketCost, lambdaCost, dynamodbCost int64) *NotificationCostTrackingBuilder {
	b.tracking.EmailCostMicroCents = emailCost
	b.tracking.PushCostMicroCents = pushCost
	b.tracking.SMSCostMicroCents = smsCost
	...
	b.tracking.TotalCostMicroCents = emailCost + pushCost + smsCost + websocketCost + lambdaCost + dynamodbCost
	return b
}
...
const (
	// Email costs (SES pricing)
	EmailCostPerMessage = 10000 // $0.10 per 1000 emails = $0.0001 per email = 10 micro-cents
	
	// Push notification costs (estimates)
	PushCostPerMessage = 5000 // $0.00005 per notification = 5 micro-cents
	
	// SMS costs (SNS pricing for US)
	SMSCostPerMessage = 645000 // $0.00645 per SMS = 645 micro-cents
...
// CalculateEmailCost calculates the cost of sending an email
func CalculateEmailCost(messageCount int64) int64 {
	return messageCount * EmailCostPerMessage
}
...
// CalculateSMSCost calculates the cost of sending SMS messages
func CalculateSMSCost(messageCount int64) int64 {
	return messageCount * SMSCostPerMessage
}
