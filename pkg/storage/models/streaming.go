...
func (s *StreamingPreferences) UpdateKeys() {
	...
	s.GSI1PK = fmt.Sprintf("USER#%s", s.Username)
	...
	if s.DeviceID != "" {
		s.GSI2PK = fmt.Sprintf("DEVICE#%s", s.DeviceID)
...
func (s *StreamingPreferences) SetDevicePreference(deviceID string) {
	s.DeviceID = deviceID
	s.SK = fmt.Sprintf("DEVICE#%s", deviceID)
	s.UpdateKeys()
}
...
// GetDefaultPreferences returns default streaming preferences for a user
