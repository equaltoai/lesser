...
func TestVerifyPortableReputation_InvalidIssuerProof(t *testing.T) {
	...
	testServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/.well-known/reputation-keys" {
			// Return a different public key than what was used to sign
	...
	// Override the issuer to point to our test server (which returns wrong key)
	pr.Issuer = testServer.URL
...
func TestVerifyPortableReputation_ValidDocument(t *testing.T) {
	...
	// Create a test server first so we know the URL
	var signer *Signer
	testServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/.well-known/reputation-keys" {
			response := map[string]string{
	...
	// Create a signer with the test server URL as the instance URL
	var err error
	signer, err = NewSigner("", testServer.URL, logger)
	require.NoError(t, err)
	...
	// Sign the document - this will set Issuer to testServer.URL
	err = signer.SignPortableReputation(pr)
	require.NoError(t, err)
	
	// Verify the issuer was set correctly
	assert.Equal(t, testServer.URL, pr.Issuer)
...
func TestGetInstancePublicKey_CachedKey(t *testing.T) {
	...
	// Create a test server that should NOT be called (key is cached)
	serverCalled := false
	testServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		serverCalled = true
		http.NotFound(w, r)
	...
	require.NoError(t, err)
	verifier.keyCache[testServer.URL] = pubKeyBytes
	
	// Call getInstancePublicKey - should return cached key without calling server
	key, err := verifier.getInstancePublicKey(context.Background(), testServer.URL)
	require.NoError(t, err)
...
func TestGetInstancePublicKey_FetchFromWellKnown(t *testing.T) {
	...
	cachedKey, exists := verifier.keyCache[testServer.URL]
...
func TestIsInstanceTrusted_NilStorageDefaultsToTrue(t *testing.T) {
	...
	// Create a test server that returns a valid public key
	var signer *Signer
	testServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/.well-known/reputation-keys" {
			response := map[string]string{
	...
	// Create signer with test server URL
	var err error
	signer, err = NewSigner("", testServer.URL, logger)
	require.NoError(t, err)
...
func TestIsInstanceTrusted_URLParsingError(t *testing.T) {
	...
	// Verify - should fail because the invalid URL can't be fetched
	result, err := verifier.VerifyPortableReputation(pr)
	require.NoError(t, err)
	require.NotNil(t, result)
	
	// The verification should fail at the key fetch stage
	assert.False(t, result.Valid, "Valid should be false for invalid issuer URL")
	assert.Contains(t, result.Error, "failed to get issuer public key")
}
...
func TestIsInstanceTrusted_DomainWithPort(t *testing.T) {
	...
	// Create signer with test server URL (which includes a port)
	var err error
	signer, err = NewSigner("", testServer.URL, logger)
	require.NoError(t, err)
...
func TestVerifyVouchSignature_ValidSignature(t *testing.T) {
	...
	// Create a test server that returns the correct public key
	var signer *Signer
	testServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/.well-known/reputation-keys" {
			response := map[string]string{
...
func TestVerifyVouchSignature_InvalidSignature(t *testing.T) {
	...
	// Create a test server that returns a DIFFERENT public key
	testServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/.well-known/reputation-keys" {
			// Return a different public key than what was used to sign
			response := map[string]string{
	...
	vouch := &Vouch{
		...
		From:        "https://test.example.com/users/alice",
		To:          "https://test.example.com/users/bob",
		InstanceURL: testServer.URL, // Points to server returning different key
		CreatedAt:   time.Now(),
		ExpiresAt:   time.Now().Add(180 * 24 * time.Hour),
		Confidence:  0.9,
		Context:     "colleague",
...
func TestVerifyVouchSignature_KeyFetchError(t *testing.T) {
	...
	vouch := &Vouch{
		...
		From:        "https://test.example.com/users/alice",
		To:          "https://test.example.com/users/bob",
		InstanceURL: testServer.URL, // Points to server that returns error
		CreatedAt:   time.Now(),
		ExpiresAt:   time.Now().Add(180 * 24 * time.Hour),
		Confidence:  0.9,
		Context:     "colleague",
