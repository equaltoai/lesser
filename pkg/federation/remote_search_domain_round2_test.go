...
func TestNormalizeActorDomainCanonicalizesAndRejectsUnsafeIPForms(t *testing.T) {
	tests := []struct {
		...
		{name: "doubled zone bare", input: "2001:db8::1%eth0%evil.com", want: "2001:db8::1"},
		{name: "mapped IPv4 with zone", input: "::ffff:1.2.3.4%evil.com", want: "1.2.3.4"},
		{name: "mapped loopback with zone", input: "::ffff:127.0.0.1%evil.com", want: ""},
		{name: "uppercase IPv6 and zone", input: "[2001:DB8::1%ETH0]", want: "2001:db8::1"},
		{name: "empty zoned host", input: "[%eth0]", want: ""},
