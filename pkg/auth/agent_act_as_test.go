...
func TestResolveActAsBlankIndicatorIsOwnerPath(t *testing.T) {
	...
	for _, indicator := range []string{"", "   "} {
		resolution, err := ResolveActAs(context.Background(), &Claims{Username: "alice"}, indicator, nil, nil)
...
func TestResolveActAsMalformedIndicator(t *testing.T) {
	...
	for _, indicator := range []string{"agent@example.com", "https://example.com/users/agent", "a/b", "a\\b", "not a user"} {
		_, err := ResolveActAs(context.Background(), &Claims{Username: "alice"}, indicator, actAsTestLookup(nil, nil), actAsTestGrantCheck(true, nil))
...
func TestResolveActAsNoActiveGrant(t *testing.T) {
	...
	_, err := ResolveActAs(context.Background(), &Claims{Username: "alice"}, "agent-one", actAsTestLookup(actAsAgentAccount(), nil), actAsTestGrantCheck(false, nil))
...
func TestResolveActAsFailsClosedOnErrors(t *testing.T) {
	...
	_, err := ResolveActAs(context.Background(), &Claims{Username: "alice"}, "agent-one", actAsTestLookup(nil, lookupErr), actAsTestGrantCheck(true, nil))
	...
	_, err = ResolveActAs(context.Background(), &Claims{Username: "alice"}, "agent-one", actAsTestLookup(actAsAgentAccount(), nil), actAsTestGrantCheck(false, grantErr))
...
func TestResolveActAsRejectsUnauthenticatedCaller(t *testing.T) {
	t.Parallel()
	...
	for _, username := range []string{"", "   ", "not a user"} {
		_, err = ResolveActAs(context.Background(), &Claims{Username: username}, "agent-one", actAsTestLookup(actAsAgentAccount(), nil), actAsTestGrantCheck(true, nil))
		require.ErrorIs(t, err, ErrActAsNotGranted, "caller %q is not a grantable identity", username)
	}
}
...
func TestResolveActAsSuccess(t *testing.T) {
	...
	resolution, err := ResolveActAs(context.Background(), &Claims{Username: "Alice"}, " Agent-One ", actAsTestLookup(actAsAgentAccount(), nil), grantCheck)
	require.NoError(t, err)
	...
	require.Equal(t, "alice", checkedGrantee, "grant check must use the normalized caller username")
}
