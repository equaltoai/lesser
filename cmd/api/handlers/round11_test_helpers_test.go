...
func round11NewHandler(t *testing.T, args ...any) (*Handler, *MockRepositoryStorage, *round10DynamoHarness) {
	...
	federationRepo := repositories.NewFederationRepository(harness.db, cfg.DynamoTableName, logger, nil, cfg)
	...
	rateLimitRepo := repositories.NewRateLimitRepository(harness.db, cfg.DynamoTableName, logger, nil)
	...
	communityNoteRepo := repositories.NewCommunityNoteRepository(harness.db, cfg.DynamoTableName, logger, nil)
	...
	repos.On("Federation").Return(federationRepo).Maybe()
	...
	repos.On("CommunityNote").Return(communityNoteRepo).Maybe()
	repos.On("Cost").Return(costRepo).Maybe()
	repos.On("OAuth").Return(oauthRepo).Maybe()
	repos.On("OAuth").Return(oauthRepo).Maybe()
	repos.On("RateLimit").Return(rateLimitRepo).Maybe()
