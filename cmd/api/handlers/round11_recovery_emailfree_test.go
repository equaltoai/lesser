...
func TestEmailFreeRecoveryOptionsAndSocialFlow(t *testing.T) {
	...
	require.NoError(t, err)
	requireStatus(t, http.StatusOK)(handler.HandleGetRecoveryOptionsLift(ctxOptions))

	ctxInit, err := round10NewLiftContext(http.MethodPost, "/auth/recovery/social/initiate", nil, nil, map[string]string{"username": "alice"})
	require.NoError(t, err)
	requireStatus(t, http.StatusOK)(handler.HandleInitiateSocialRecoveryLift(ctxInit))
	...
	require.NoError(t, err)
	requireStatus(t, http.StatusOK)(handler.HandleConfirmSocialRecoveryLift(ctxConfirm))
}
...
func TestEmailFreeRecoveryCodesAndDevices(t *testing.T) {
	...
	require.NoError(t, err)
	ctxGenerate.Set("jwt_claims", map[string]any{"sub": "alice"})
	requireStatus(t, http.StatusOK)(handler.HandleGenerateRecoveryCodesLift(ctxGenerate))
	...
	require.NoError(t, err)
	requireStatus(t, http.StatusOK)(handler.HandleUseRecoveryCodeLift(ctxUse))
	...
	require.NoError(t, err)
	ctxTrusteeAdd.Set("jwt_claims", map[string]any{"sub": "alice"})
	requireStatus(t, http.StatusOK)(handler.HandleAddTrusteeLift(ctxTrusteeAdd))

	ctxTrustees, err := round10NewLiftContext(http.MethodGet, "/auth/recovery/trustees", nil, nil, nil)
	require.NoError(t, err)
	ctxTrustees.Set("jwt_claims", map[string]any{"sub": "alice"})
	requireStatus(t, http.StatusOK)(handler.HandleListTrusteesLift(ctxTrustees))

	ctxRemove, err := round10NewLiftContext(http.MethodDelete, "/auth/recovery/trustees/trustee-1", nil, nil, nil)
	require.NoError(t, err)
	ctxRemove.Set("jwt_claims", map[string]any{"sub": "alice"})
	ctxRemove.Params["trustee_id"] = "@bob@example.com"
	requireStatus(t, http.StatusOK)(handler.HandleRemoveTrusteeLift(ctxRemove))
	...
	require.NoError(t, err)
	requireStatus(t, http.StatusOK)(handler.HandleDeviceRecoveryLift(ctxDevice))
}
