...
func usernameFromJWTClaims(ctx *apptheory.Context) string {
	if ctx == nil {
		return ""
	}
	claims, ok := ctx.Get("jwt_claims").(map[string]any)
	if !ok {
		return ""
	}
...
func (h *EmailFreeRecoveryHandler) HandleGetRecoveryOptionsLift(ctx *apptheory.Context) (*apptheory.Response, error) {
	username := queryValue(ctx, "username")
	if err := common.ValidateRequiredParam("username", username); err != nil {
		return apptheory.JSON(http.StatusBadRequest, map[string]any{"error": err.Error()})
	}
...
func (h *EmailFreeRecoveryHandler) HandleInitiateSocialRecoveryLift(ctx *apptheory.Context) (*apptheory.Response, error) {
	...
	if err := common.ParseRequestWithFallback(ctx, &req); err != nil {
		return apptheory.JSON(http.StatusBadRequest, map[string]any{"error": "invalid request body"})
	}
...
func (h *EmailFreeRecoveryHandler) HandleGenerateRecoveryCodesLift(ctx *apptheory.Context) (*apptheory.Response, error) {
	...
	username := usernameFromJWTClaims(ctx)

	if err := common.ValidateRequiredParam("username", username); err != nil {
		return apptheory.JSON(http.StatusUnauthorized, map[string]any{"error": "authentication required"})
	}
	...
	if err != nil {
		...
		return apptheory.JSON(http.StatusInternalServerError, map[string]any{"error": "failed to generate codes"})
	}
...
func (h *EmailFreeRecoveryHandler) HandleUseRecoveryCodeLift(ctx *apptheory.Context) (*apptheory.Response, error) {
	...
	if err != nil || !valid {
		return apptheory.JSON(http.StatusBadRequest, map[string]any{"error": "invalid recovery code"})
	}
	...
	if err != nil {
		...
		return apptheory.JSON(http.StatusInternalServerError, map[string]any{"error": "internal server error"})
	}
...
func (h *EmailFreeRecoveryHandler) HandleAddTrusteeLift(ctx *apptheory.Context) (*apptheory.Response, error) {
	...
	if !strings.Contains(req.TrusteeActorID, "@") {
		return apptheory.JSON(http.StatusBadRequest, map[string]any{"error": "invalid actor ID format, expected @username@instance"})
	}
...
func (h *EmailFreeRecoveryHandler) HandleDeviceRecoveryLift(ctx *apptheory.Context) (*apptheory.Response, error) {
	...
	if err != nil || device.Username != req.Username || device.TrustLevel != "trusted" {
		return apptheory.JSON(http.StatusBadRequest, map[string]any{"error": "device not found or not trusted"})
	}
